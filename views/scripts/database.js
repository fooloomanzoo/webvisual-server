	...
	ClientDatabase.prototype = {
		...
		get: function(start, end, limit, direction) {
			...
			return this.dbPromise.then(function(db) {
				const tx = db.transaction(this.indexKey);
				const values = [];
				const store = tx.objectStore(this.indexKey);
				let keyRange = null;
		...
		set: function(data) {
			return this.dbPromise.then(function(db) {
				const tx = db.transaction(this.indexKey, 'readwrite');
		...
		range: function() {
			return new Promise(function(resolve, reject) {
				const ret = [null, null];
				this.first(null, null, 1).then(function(f) {
					ret[0] = f[this.mount][0][this.indexKey];
					this.last(null, null, 1).then(function(l) {
