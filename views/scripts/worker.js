...
IOSocket.prototype = {
	...
	connect: function(opt) {
		if (!this.socket) {
			...
			this.socket = io.connect(this.locationHost + '/' + this.nameSpace, {multiplex: false});
			...
			this.socket.on('error', function(err) {
				...
				this._connectionLostAt = +(new Date());
				self.postMessage({type: 'status', status: 'sync-problem'})
			}.bind(this));
	...
	setup: function(opt) {
		...
		if (this.socket && opt.socketRoom) {
			const facility = opt.socketRoom.split('/')[0],
...
const CacheStore = new Store('cache', 'x'),
	...
	switch (e.data.target) {
		...
		case 'database':
			if (DatabaseStore[e.data.operation]) {
				DatabaseStore[e.data.operation](e.data.args).then(function(res) {
					console.log(res);
					self.postMessage({type: 'request', messageId: e.data.args.messageId, response: res});
				}).catch(function(err) {
					if (err) {
						console.warn(err);
					}
					self.postMessage({type: 'request', messageId: e.data.args.messageId, response: {}});
				});
		...
		case 'cache':
			if (CacheStore[e.data.operation]) {
				CacheStore[e.data.operation](e.data.args).then(function(res) {
					self.postMessage({type: 'request', messageId: e.data.args.messageId, response: res});
				}).catch(function(err) {
					if (err) {
						console.warn(err);
...
self._updateClient = function(message, type) {
	...
	for (let mount in message.values) {
