(function (root, factory) { // 15.70 KB, 8.42 KB, 3.23 KB
	...
	function toggle(_this, e) {
		...
		if ([].indexOf.call(options.elements, target) !== -1) {
			...
			_this.date = getDateTime(_this, options, target);
			_this.isOpen = true;
			_this.toggled = _this.currentInput !== target;
			_this.currentInput = target;
			_this.currentPartner = getPartner(_this, target);
			_this.currentDate = getDateObject(assembleDate(_this.date)); // is new object

			addLimiters(_this, options, target, _this.currentPartner, id);
			renderDatePicker(_this, target, _this.date);
			_this.toggled = false;
