...
class ConfigFileProcessor extends EventEmitter {
    ...
    // try {
  		...
  		let items = []
  		  , itemMap = {}
  		  , item
  		  , keys = locals.unnamedItem && locals.unnamedItem.keys ? Object.keys(locals.unnamedItem.keys) : [];
        ...
        i++;
  		...
  		for (let id in itemMap) {
  			if (itemMap[id] && itemMap[id].svg && itemMap[id].svg.source) {
  				...
  				if (!svgSource.source)
  					svgSource.source = {};
  				if (!svgSource.source[source])
  					svgSource.source[source] = {title:{}, style:{selectable:'', selected:'', exceeding:''}};
  				if (!svgSource.source[source].title)
  					svgSource.source[source].title = {};
  				if (itemMap[id].svg.path)
  					svgSource.source[source].title[id] = itemMap[id].svg.path;
  			}
  		}
