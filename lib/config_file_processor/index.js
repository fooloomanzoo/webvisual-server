...
class ConfigFileProcessor extends EventEmitter {
	...
	unwatch() {
		for (let facility in this.filehandler) {
			...
			this.config[facility].dataConfig = null;
			delete this.config[facility].dataConfig;
    ...
    let views = ['list', 'groups', 'detail'];
        ...
        this.config[facility][system]._view = (data[i].initialView && views.indexOf(data[i].initialView) !== -1) ? data[i].initialView : 'list';

        this.config[facility][system].connection = data[i].connections;

        if (data[i].database) {
          this.config[facility][system].database = data[i].database;
        ...
        this.config[facility][system].cache = data[i].cache;
        this.config[facility][system].valueModel = data[i].valueModel || defaultValueModel;
        this.config[facility][system].svgSource = data[i].svgSource || {};
        ...
        for (let key in dataConfig) {
          this.config[facility][system][key] = dataConfig[key];
        }
			}
		} catch(err) {
    ...
    // try {
      ...
      for (var key in defaults) {
        if (unnamedItem[key] === undefined) {
          ...
          if (item.svg && item.svg.path) {
            filePath = path.parse(item.svg.path);
            if (!(item.svg.selector && filePath.base && filePath.dir)) {
              delete item.svg;
            } else {
              item.svg.path = filePath.base;
              item.svg.dir = filePath.dir;
            }
          }
          ...
          'groups': []
  			...
  			for (let subgroup in groupMap[group]) {
  				...
  				for (let id of groupMap[group][subgroup].items) {
  					if (itemMap[id] && itemMap[id].svg && itemMap[id].svg.path && itemMap[id].svg.dir) {
  						if (p && p !== itemMap[id].svg.path && dir && dir !== itemMap[id].svg.dir) {
  							sameSource = false;
  							break;
  						} else {
  							p = itemMap[id].svg.path;
  						}
  					}
  				}
  				if (sameSource) {
      ...
      if (!svgSource.paths)
        svgSource.paths = {};
      let base;
      for (let p in svgSource.paths) {
        base = path.basename(p)
        if (p !== base) {
          svgSource.paths[base] = svgSource.paths[p]
          delete svgSource.paths[p]
        }
      }
  		...
  		for (let id in itemMap) {
  			if (itemMap[id] && itemMap[id].svg && itemMap[id].svg.path) {
  				p = itemMap[id].svg.path
  				if (!svgSource.paths[p])
  					svgSource.paths[p] = {items:{}, style:{selectable:'', selected:'', exceeding:'', clicked:''}};
  				if (!svgSource.paths[p].items)
  					svgSource.paths[p].items = {};
  				if (itemMap[id].svg.selector)
  					svgSource.paths[p].items[id] = itemMap[id].svg.selector;
  			}
  		}
