...
class ConfigFileProcessor extends EventEmitter {
    ...
    let views = ['list', 'groups', 'detail'];
        ...
        this.settings[facility][system]._view = (data[i].initialView && views.indexOf(data[i].initialView) !== -1) ? data[i].initialView : 'list';
        ...
        if (data[i].database) {
          ...
          if (this.databaseConfig) {
            for (var key in this.databaseConfig) {
              if (this.settings[facility][system].database[key] === undefined) {
                this.settings[facility][system].database[key] = this.databaseConfig[key];
              }
            }
          }
        } else if (this.databaseConfig) {
          this.settings[facility][system].database = this.databaseConfig;
        }
