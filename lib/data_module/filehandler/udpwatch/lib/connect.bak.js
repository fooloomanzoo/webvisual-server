...
function _create_watch_options(mode, options) {
  ...
  // Check if process/emitter are valid
  ...
  } else if(options.process || options.emitter) {
...
function _handle_change(event, path, currStat, prevStat, options) {
  ...
  if (event === 'update' || event === 'create') {
    if (options.mode === 'append') {
      options.work_function(path, prevStat.size, options.process, options.emitter);
    } else if (options.mode === 'prepend') {
      options.work_function(path, 0, options.process, options.emitter);
    } else if (options.mode === 'all') {
      options.work_function(path, undefined, options.process, options.emitter);
    }
  }
  ...
  {options}
    copy - a boolean which states if copywatch should make a copy
      ...
      false - the file won't be copied.
        You will have to give copywatch a emitter-function (a process-function is optional);
...
function watch(mode, port, options, next) {
  ...
  fs.exists(resFile, function(exists) {
    if(exists === false) {
      ...
      options.work_function(resFile, undefined, undefined, options.process, options.emitter);
    }
  });
