...
/* Runs a parser_msg function with upgraded callback,
 * that additionally writes the message to the log-file.
 * */
function _parser_log(message, peer, options, callback) {
  ...
  _parser_msg(message, peer, options, finish);
}

/* Processes the message and uses the callback with parsered data. */
function _parser_msg(message, peer, options, callback) {
if(callback === undefined) return; //this function is useless without a callback
  ...
  if(tokens.length === 1) {
    ...
    for(var i=0; i<tokens.length; ++i) {
      // Skip empty lines
      if(tokens[i].length > 0) options.parser(tokens[i], pushData);
  ...
  if(tmpBuffer !== "") {
    options.parser(tmpBuffer, pushData);
  }

  // Are there any errors?
  if(errorData.length === 0) errorData = null;

  callback(errorData, parseredData);
}
...
function _create_watch_options(options) {
  ...
  if(options.log) {
    ...
    nOptions.work_function = _parser_msg;
  } else {
...
/*
  Watch
    ...
    {options}
       ATTENSION: If there is not emitter-function and log is false than connect
         will throw an error, since there is no point in watching a file and
         doing nothing on change.
      emitter - a function that becomes a message data, either parsed (if a
        parser function was given) or in raw form, every time a change happens.
      parser - a function which parseres each line from a received message.
        It need to take the following argument: 'string' (the current line),
        'callback' (optional - a function that receives the parsered line.
        Otherwise it will be assumed, that the function returns the data.)
        Arguments are err (an potential array of errors) and data
        (an array of the (parsered) lines).
...
function watch(port, options) {
  ...
  _watchers[port].on('message', function(message, info) {
    ...
    options.work_function(message, _lastPeer, options, options.emitter);
  });
