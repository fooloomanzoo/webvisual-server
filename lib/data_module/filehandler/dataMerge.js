...
function processData(data, facility, system, settings) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		for (var j = 0; j < data[i].values.length; j++) {
			...
			if (settings.ignore.indexOf(j) === -1 && k < settings.ids.length) {
				// if it didn't exist before in process for return
				id = settings.ids[k].id || settings.unnamedType.id + k;
        mount = settings.ids[k].mount || facility + '/' + system + '/' + id;
				...
				if (data[i].values[j] !== null) {
					// exceeding
					if (settings.ids[k].threshold !== undefined) {
						if (settings.ids[k].threshold.from !== undefined &&
							data[i].values[j] < settings.ids[k].threshold.from)
							state = -1;
						else if (settings.ids[k].threshold.to !== undefined &&
							data[i].values[j] > settings.ids[k].threshold.to)
							state = 1;
					}
				}
