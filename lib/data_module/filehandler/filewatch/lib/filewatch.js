...
(function() {
  ...
  const fs = require('fs'),
    ...
    DEFAULT = {
      ...
      ignoredFirstLines: 0,
      waitIntervalForRecreation: 2000
    },
    _watchers = new Map(),
    _extension = '.log',
    NEW_LINE = /\r\n|\n\r|\n|\v/, // Every possible NEW_LINE character
    ERROR_FILE = './filewatch.err',
  ...
  */
  function _process_log(path, start, end, parser, callback, log_path) {
    // Define Variables
    ...
    _process_read(path, start, end, parser, finish);
  }

  // Read Process
  function _process_read(path, start, end, ignored, parser, callback) {
    // Define variables
    var processedData = [],
      rawData = [],
      errorData = [],
  ...
  // Read a json file
  function _process_read_json_file(path, start, end, parser, callback) {
    var processedData = {},
      errorData = [];
  ...
  */
  function _create_watch_options(mode, filepath, options) {
    var nOptions = {
      ...
      log_path: path_util.join(options.log_path, filepath),
      ...
      waitIntervalForRecreation: options.waitIntervalForRecreation || DEFAULT.waitIntervalForRecreation
    };
    ...
    } else if (options.parser || options.emitter) {
  ...
  function _handle_change(event, path, currStat, prevStat, options) {
    ...
    if (options.mode === 'append') {
      if (currStat < prevStat) {
        ...
        if (event === 'add')
          options.work_function(path, prevStat, currStat, options.ignoredFirstLines, options.parser, options.emitter, options.log_path);
        else
          options.work_function(path, prevStat, currStat, 0, options.parser, options.emitter, options.log_path);
      }
      ...
      if (currStat < prevStat) {
        ...
        if (event === 'add')
          options.work_function(path, 0, (currStat - prevStat), options.ignoredFirstLines, options.parser, options.emitter, options.log_path);
        else
          options.work_function(path, 0, (currStat - prevStat), 0, options.parser, options.emitter, options.log_path);
      }
    } else if (options.mode === 'all') {
      options.work_function(path, undefined, undefined, options.ignoredFirstLines, options.parser, options.emitter, options.log_path);
    } else if (options.mode === 'json'){
      options.work_function(path, undefined, undefined, options.parser, options.emitter, options.log_path);
    }
  }
  ...
  */
  ...
  function watch(mode, filepath, options, callback) {
    // Define variables
    ...
    maybeError = _check_file(filepath);
    if (maybeError) throw maybeError;
    ...
    options = _create_watch_options(mode, filepath, options || {});
    ...
    const resFile = path_util.resolve(filepath);
    let active_wait_job = null;

    // function to start the file watching
    var watch_the_file = function(path) {
      // watch the file
      if (_watchers.has(path)) {
        unwatch(path, null, callback);
      }
      let watcher = new chokidar.watch(path, options.watch_settings)
        .on('add', (_path, stats) => {
          console.log(`File ${_path} is being added to watch`);
          _handle_change('add', _path, stats.size, 0, options);
          _watchers.get(_path).prevStat = stats.size;
        })
        .on('change', (_path, stats) => {
          // if (stats)
          //   console.log(`File ${path} changed size from ${_watchers.get(path).prevStat} to ${stats.size}`);
          _handle_change('change', _path, stats.size, _watchers.get(_path).prevStat, options);
          _watchers.get(_path).prevStat = stats.size;
        })
        .on('unlink', _path => console.log(`File ${_path} has been removed or is not present`))
        .on('error', error => {
          if (!active_wait_job) {
            console.log(`File Error (module: filewatch.js, chokidar): ${error}`);
            wait_until_created(path);
          }
        });
    ...
    var wait_until_created = function(path) {
      if (active_wait_job) {
        clearInterval(active_wait_job);
      }
      active_wait_job = setInterval(() => {
        fs.exists(path, exists => {
          if (exists === true) {
            clearInterval(active_wait_job);
            active_wait_job = null;
            // file was created -> start watching
            watch_the_file(path);
          }
        });
      }, options.waitIntervalForRecreation)
    }

    //Check for existence of directory (initially)
    fs.exists(resFile, function(exists) {
      if (exists === false) { // If file doesn't exists -> no reason to start the watcher
        console.warn(`File "${resFile}" is not found. filewatch now listens for creation.`);
        wait_until_created(resFile);
      } else {
        watch_the_file(resFile);
      }
    });
  }
