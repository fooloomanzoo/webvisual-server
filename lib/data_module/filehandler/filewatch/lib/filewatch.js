  ...
  */
  function _create_watch_options(mode, options) {
    var nOptions = {
      ...
      watch_error: options.emitter || _default.watch_error,
  ...
  function watch(mode, file, options, callback) {
    ...
    var watch_the_file = function(path) {
      ...
      let watcher = new chokidar.watch(path, options.watch_settings)
        ...
        .on('unlink', _path => console.log(`File ${_path} has been removed`))
        .on('error', error => {
