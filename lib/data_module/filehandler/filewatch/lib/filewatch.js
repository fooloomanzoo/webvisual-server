...
(function() {
  ...
  */
  function _log(path, start, end, processor, emitter, log_path) {
  ...
  */
  function _create_watch_options(mode, options) {
    ...
    // Check if processor/emitter are valid
    ...
    } else if (options.processor || options.emitter) {
  ...
  function _handle_change(event, path, currStat, prevStat, options) {
    ...
    if (options.mode === 'append') {
      if (currStat < prevStat) {
        ...
        if (event === 'add')
          options.work_function(path, prevStat, currStat, options.ignoredFirstLines, options.processor, options.emitter, options.log_path);
        else
          options.work_function(path, prevStat, currStat, 0, options.processor, options.emitter, options.log_path);
      }
      ...
      if (currStat < prevStat) {
        ...
        if (event === 'add')
          options.work_function(path, 0, (currStat - prevStat), options.ignoredFirstLines, options.processor, options.emitter, options.log_path);
        else
          options.work_function(path, 0, (currStat - prevStat), 0, options.processor, options.emitter, options.log_path);
      }
    } else if (options.mode === 'all') {
      options.work_function(path, undefined, undefined, options.ignoredFirstLines, options.processor, options.emitter, options.log_path);
    } else if (options.mode === 'json'){
      options.work_function(path, undefined, undefined, options.processor, options.emitter, options.log_path);
    }
  }
  ...
  function watch(mode, file, options, callback) {
    ...
    var watch_the_file = function() {
      if (options.firstLog) {
        ...
        options.work_function(resFile, undefined, undefined, options.processor, options.emitter);
      }
