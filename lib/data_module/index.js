	...
	setServer(server) {
		...
		this.dataSocket.on('connection', (client) => {
			...
			client.on('setup', (config) => {
				const roomName = config.room
						, facility = roomName.split('/')[0]
						, system = roomName.split('/')[1];
						
				if (!roomName || !facility || !system)
					return;
				...
				client.compress(true).emit('initial', {
					facility: facility,
					system: system,
					values: this.cache[facility][system].request(requestlast)
				});

				client.join(roomName); // client joins room for selected system
			});
			...
			let listeners = {
				...
				data: (option, data, system) => {
					...
					if (this.settings[facility] && this.settings[facility][system]) {
						...
						this.dataSocket.to(facility + '/' + system).emit('update', mergedData);
					}
				}
			};
