  ...
  setServer(io) {
    ...
    this.dataSocket.on('connection', (client) => {
      ...
      client.on('request', (config) => {
        if (!config || !config.room || !config.messageId || !config.mounts) {
          return;
        }
        ...
        if ( !room
          ...
          this.db[facility][system].getAll(config.from || null, config.to || null, config.limit || 30000, config.mounts)
            .then( (res) => {
              client.compress(true)
                .emit('request', {
                  messageId: config.messageId,
