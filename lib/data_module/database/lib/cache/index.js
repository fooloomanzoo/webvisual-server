...
class CacheClient extends DBInterface {
  ...
  get(mount, start, stop, limit = this.options.maxCount) {
    return new Promise( (resolve, reject) => {
      ...
      if (this._cache.has(mount)) {
        let cacheEntry = this._cache.get(mount);

        if (!cacheEntry) {
          resolve(ret);
        }

        if (start === null || start === undefined || start >= limit) {
          start = 0;
          stop = -1;
        } else if (!stop || (stop - start) > limit) {
          stop = -1;
        }

  			ret.values = cacheEntry.slice(start, stop);
      } else {
  ...
  delete(mount, min, max) {
    return new Promise( (resolve, reject) => {
      ...
      resolve(this.deleteByIndex(mount, startIndex, endIndex));
    });
  }

  deleteByIndex(mount, start, end) {
    return new Promise( (resolve, reject) => {
      if (this._cache.has(mount) && start !== undefined && end !== undefined) {
        this._cache.get(mount).splice(start, end);
      }
      resolve();
    });
  }
