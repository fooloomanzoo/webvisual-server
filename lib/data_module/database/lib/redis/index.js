...
class RedisClientDB extends DBInterface {
  ...
  _cleanUp(maxCount) {
    if (this.mounts) {
      this.mounts.forEach( (mount) => {
        this.count(mount)
            .then( (count) => {
              this.deleteByIndex(mount, null, null, count - maxCount)
  ...
  get(mount, start, stop, limit) {
    return new Promise( (resolve, reject) => {
      if (this.mounts && this.mounts.has(mount)) {
        // start on maximum values, because they are the newest
        this.client.zrevrangebyscore(mount, [start || Number.MAX_SAFE_INTEGER, stop || Number.MIN_SAFE_INTEGER, 'limit', 0, limit], (err, result) => {
          if (err) {
            this.emit('error', `${this.dbName}: (${this.options.type})\n${err}`);
          }
          var ret = [];
  ...
  deleteByIndex(mount, min, max, limit) {
    return new Promise( (resolve, reject) => {
      this.client.zremrangebyscore(mount, [min || Number.MIN_SAFE_INTEGER, max || Number.MAX_SAFE_INTEGER, 'limit', 0, limit], (err, result) => {
        if (err)
          this.emit('error', `${this.dbName}: (${this.options.type})\n${err}`);
        resolve(result || 0);
      })
