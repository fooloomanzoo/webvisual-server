...
class processEmiter {
  ...
  setEmitter() {
      ...
      process.send = (arg) => {
        for (var type in arg) {
          switch (type) {
            case 'event':
            case 'info':
              this.emitter.info(type + ': ' + (arg[type] || JSON.stringify(arg)))
              break
            case 'error':
              if (!arg.error) {
                try {
              ...
              this.emitter.error('error: ' + arg.error)
              break
            case 'warn':
              this.emitter.warn('warn: ' + (arg.warn || JSON.stringify(arg)))
              break
            case 'log':
            default:
              this.emitter.log(type + ': ' + (arg[type] || JSON.stringify(arg)))
          }
        }
      }
    ...
    process.on('uncaughtException', (err) => {
      ...
      if (err) {
        this.emitter.error(err.stackTrace || err.stack || err)
      }
      if (this.restartJob) {
        clearTimeout(this.restartJob)
        this.restartJob = null
