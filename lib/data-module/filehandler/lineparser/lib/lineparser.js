...
function  _toCharCode(str) {
  return [...str].map(c => {return String.prototype.charCodeAt.apply(c)})
}
			...
			for (let i = 0; i < values.length; i++) {
				...
				if (!isNaN(check_val)) {
					...
					if (nullValue.test(values[i])) {
						...
						throw new Error('LineParser: String includes invalid numbers: ' + values[i] + ' (charcodes: ' + _toCharCode(values[i]) + ', i: ' + i + ')\n' + str);
					}
				}
