...
class LineParser {
   ...
   * @param  {[number]}   line   [linenumber]
   * @param  {Function} callback [callback function]
   */
  run(str, line, callback) {
    // Define Variables
    const data = {
      date: undefined,
      values: []
    ...
    if (typeof str !== 'string') {
      callback(`LineParser: argument has to be from type string.`, line);
      return;
    }
    // if (!str) {
    ...
    if (this.options.dateFormat === 'iso') {
      ...
      if (check_val === -1) {
        callback(`LineParser: String doesn't match given dateFormat: ${this.options.dateFormat}`, line);
        return;
      }
    ...
    if (isNaN(data.date)) {
      callback(`LineParser: invalid Date`, line);
      return;
    }
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (!isNaN(check_val)) {
        ...
        if (nullValue.test(values[i])) {
          ...
          callback('LineParser: String includes invalid numbers', line);
          return;
        }
      }
    }
    callback(null, data, this.passRawData ? str : null);
  }
