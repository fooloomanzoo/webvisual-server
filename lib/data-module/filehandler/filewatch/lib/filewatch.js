...
(function() {
  ...
  const fs = require('fs'),
    ...
    RELEASE_LENGTH = 2000; // Maximum size of collected rows, until they are released for processing
  // Functions
  ...
  */
  function _check_file(path) {
    if (typeof path !== 'string') {
      return new Error('File path must be a String, '+(typeof path));
    }
    return null;
  }
  ...
  */
  function _log(path, start, end, parser, emitter, log_path) {
    const options = _file_options(start, end);
    // Copies the file
  ...
  */
  function _process_log(path, start, end, ignoredFirstLines, parser, callback, finish, log_path) {
    ...
    const options = _file_options(start, end).writeOptions;
    ...
    function write(errorData, processedData) {
      ...
      const data = _watchers.get(path).data || [];
      let arrFn;
      ...
      const writeLog = fs.createWriteStream(log_path + _extension, options);

      // Write the data and close the stream
      writeLog.end(JSON.stringify(data));
  ...
  function _process_read(path, start, end, ignoredFirstLines, parser, callback, finish) {
    ...
    const read = fs.createReadStream(path, readOptions);
    ...
    read.on('readable', function() {
      ...
      // Split the data
      const tokens = data.split(NEW_LINE);
      ...
      for (i = ignoredFirstLines; i < tokens.length; ++i) {
        // Skip empty lines
        if (/\S/.test(tokens[i])) {
          // string is not empty and not just whitespace
          parser.run(tokens[i], i, function(err, data, raw) {
            if (err) {
              errorData.push({
                path: path,
                line: data,
              ...
              if (callback && processedData.length > RELEASE_LENGTH) {
                // clear stashed data from memory by releasing
                callback(null, processedData, rawData);
    ...
    read.on('end', function() {
      ...
      if (tmpBuffer !== '') {
        parser.run(tmpBuffer, i, function(err, data, raw) {
          if (err) {
            errorData.push({
              path: path,
              line: data,
  ...
  function _process_read_json_file(path, start, end, ignoredFirstLines, parser, callback, finish) {
    ...
    fs.readFile(path, (err, data) => {
      ...
      try {
        ...
        if (callback) callback(errorData.length === 0 ? null : errorData, processedData);
        if (finish) finish();
      }
    });
  }
    ...
    if (options.mode === 'append') {
      ...
      if (options.mode === 'once') {
        if (!watchpath.match(GLOB_PATTERN)) {
          ...
          const watcher = _watchers.get(watchpath);
    ...
    _watchers.set(watchpath, new chokidar.watch(watchpath, options.watch_settings)
      ...
      .on('error', error => {
        if (restartJob) {
          ...
          console.log(`File Error (module: filewatch.js, chokidar): ${error.stack}`);
        }
