...
// "Global" variables
  _default    = {
    firstCopy: true,
    parser: function(string, callback) {
      callback(null, string);
    },
    work_function: _parser_copy,
...
/*
  Process copy
  Copies a data and parseres it, if a parser function is given.
*/
function _parser_copy(path, start, end, parser, callback) {
  // Define Variables
  ...
  function finish(errorData, parseredData) {
    ...
    // Make the callback
    if(callback) callback(errorData, parseredData);
  }

  _parser_read(start, end, parser, finish);
}

function _parser_read(start, end, parser, callback) {
  // Define variables
  var parseredData = [],
    errorData     = [],
    encoding = 'utf8',
  ...
  read.on('readable', function() {
    ...
    for(var i=0; i<tokens.length; ++i) {
      // Skip empty lines
      if(tokens[i].length > 0) parser(tokens[i], pushData);
...
function _create_watch_options(mode, options) {
  var nOptions =  {
    ...
    parser: options.parser || _default.parser,
  ...
  if(options.parser && !isFunction(options.parser)) {
    return new TypeError('The parser-option needs to be an function.');
  }
  ...
  if(options.copy === false) {
    ...
    if(options.emitter) {
      // Just parser the file and give the data to the specified callback
      nOptions.work_function = _parser_read;
    } else {
    ...
    nOptions.work_function = _parser_copy;
  }
...
function _handle_change(event, path, currStat, prevStat, options) {
  // Test/create event - parser the changes
  if (event === 'update' || event === 'create') {
    if (options.mode === 'append') {
      options.work_function(path, prevStat.size, options.parser, options.emitter);
    } else if (options.mode === 'prepend') {
      options.work_function(path, 0, options.parser, options.emitter);
    } else if (options.mode === 'all') {
      options.work_function(path, undefined, options.parser, options.emitter);
    }
  }
...
function _create_listeners(options) {
  return {
    ...
    change: function (event, path, currStat, prevStat) {
      // If its an event for a file we don't watch, there is no reason to parser it; this should actually never happen it's just an extra ensurance
  ...
  {options}
    copy - a boolean which states if copywatch should make a copy
      ...
      false - the file won't be copied.
        You will have to give copywatch a emitter-function (a parser-function is optional);
        the emitter-function will recieve the file-data on change, so you can work with it.
        If there is not emitter-function than copywatch will throw an error, since there is
    ...
    parser - a function which parseres each line which is read from the file when a change occures.
      The parsered data will be saved as JSON in an array with every line, so it's easier to reread it from the file.
      It need to take the following argument: 'string' (the current line), 'callback' (optional - a function
      that recieves the parsered line. Otherwise it will be assumed, that the function returns the data.)
    emitter - a function that recieves the whole emitter of the file, either parsed (if a parser function was given) or in raw form,
      every time a change happens.
      Arguments are err (an potential array of errors) and data (an array of the (parsered) lines).
...
function watch(mode, port, options, next) {
  ...
  fs.exists(resFile, function(exists) {
    if(exists === false) {
      ...
      options.work_function(resFile, undefined, undefined, options.parser, options.emitter);
    }
  });
...
module.exports = {
  ...
  _copy: _copy,
  _parser_copy: _parser_copy,
  _parser_read: _parser_read,
