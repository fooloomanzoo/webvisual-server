...
class RedisClientDB extends DBInterface {
  ...
  constructor(dbName, options, data) {
    ...
    this.options.valueKeys = Object.keys(this.options.valueSchema);
  }
    ...
    return new Promise( (resolve, reject) => {
        ...
        this.client.zrevrangebyscore(mount, [to || Number.MAX_SAFE_INTEGER, from || Number.MIN_SAFE_INTEGER, 'withscores', 'limit', 0, limit || Number.MAX_SAFE_INTEGER], (err, result) => {
          ...
          for (var i = 0; i < result.length; i+=2) {
            ret.unshift( this.decodeData(result[i+1], result[i]) );
          }
  ...
  encodeData(value) {
    var ret = '';
    for (var key of this.options.valueKeys) {
      ret += key + this.options.keySeperator + value[key] + this.options.valueSeparator;
    }
    return ret;
  }

  decodeData(index, str) {
    var ret = {}, tmp, parts;
    parts = str.split(this.options.valueSeparator).filter(p => { return p !== '' });
    ret[this.options.indexKey] = index;
    for (var part of parts) {
      tmp = part.split(this.options.keySeperator);
      ret[tmp[0]] = +tmp[1];
    }
    // console.log(ret);
    return ret;
