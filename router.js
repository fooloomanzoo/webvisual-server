...
const requiredStaticSettings = ['groups', 'groupingKeys', 'preferedGroupingKey', 'svgSource', 'items'];

const dirSources = path.join(__dirname, 'views'),
	dirProduction = {
    compiled: path.join(__dirname, 'views/build/compiled'),
    bundled: path.join(__dirname, 'views/build/bundled')
  },
  dir = {
  	locales: path.join(__dirname, 'views/locales'),
  	icons: path.join(__dirname, 'views/icons'),
  	fonts: path.join(__dirname, 'views/fonts')
  };

const REGEXP_FILEPATH = /.*\/.+\..{1,}$/;
...
class Router extends EventEmitter {
	...
	setApp(options, sslSettings) {
		...
		this.dirTmp = this.settings.server._tmpDir
			? resolvePath(this.settings.server._tmpDir)
		...
		switch (this.mode) {
			case 'development':
				let srcpath = resolvePath(dirSources);
		...
		this.app.post('/login', this.multer.fields([]), this.settings.server.auth.required
			? this.passport.authenticate('activedirectory-login')
			: this.passport.authenticate('dummy'), (req, res) => {
			res.status(200).send(req.user);
		});

		// Auth Test
		this.app.use('/auth', this.settings.server.auth.required
			? ensureLoggedIn.isRequired
			: ensureLoggedIn.notRequired, (req, res) => {
		...
		// Secured Data
		this.app.use('/locales', serveStatic(dir.locales, {
			index: false,
			fallthrough: false
		}))
		this.app.use('/fonts', serveStatic(dir.fonts, {
			index: false,
			fallthrough: false
		}))
		this.app.use('/icons', serveStatic(dir.icons, {
			index: false,
			fallthrough: false
		}))

		// Secured Data
		this.app.use('/data', this.settings.server.auth.required
			? ensureLoggedIn.isRequired
			: ensureLoggedIn.notRequired);
		this.app.use('/data', serveStatic(this.dirData, {
			index: false,
			fallthrough: false
		}));

		this.app.use('/images', this.settings.server.auth.required
			? ensureLoggedIn.isRequired
			: ensureLoggedIn.notRequired);
		...
		this.app.get('*', this.serveResources.bind(this));
    ...
    resolvePushManifests(dirSources, dirProduction)
      .then(data => {
        this.pushManifests = data;
      });
	...
	createStaticContent() {
		...
		for (let facility in this.configurations) {
			...
			for (let system in opt) {
				...
				tmp.push({name: opt[system]._name, title: opt[system]._title, view: opt[system]._view});
				...
				promises.push(new Promise((resolve, reject) => {
					mkdirp(svgDest, error => {
						if (error)
							reject(`SVG-File Destination folder failed to create \n ${error.stack}`);
						resolve();
					});
				}));
					...
					if (item.hasOwnProperty('svg')) {
						if (item.svg.path && item.svg.dir) {
							...
							if (origin && dest && !toCopy.has(dest)) {
								toCopy.add(dest);
								delete item.svg.dir;
								promises.push(copy(origin, dest));
							}
						}
					}
				});
			...
			if (tmp && tmp.length > 0) {
				facilities.push({name: this.configurations[facility]._name, title: this.configurations[facility]._title, systems: tmp});
			}
		}
  ...
  serveResources(req, res, next) {
    const build = (this.mode === 'development') ? '' : (useragent_supports_es6(req) ? 'bundled' : 'compiled');
  	const root = (build === '') ? dirSources : dirProduction[build];
  	const reqPath = req.path;
  	const entrypoint = 'index.html';
    const resources = (build === '') ? this.pushManifests.development[reqPath] : this.pushManifests.production[build][reqPath];
  	// http2-server-push
  	// if (res.push && !req.get('x-is-push') && resources) {
    //   console.log(resources, reqPath)
    //   for (const filename in resources) { // push-manifest for build
    //     console.log(path.join(!resources[filename].root ? root : dirSources, filename));
    ...
    //     fs.createReadStream(path.join(resources[filename].root ? dirSources : root, filename)).pipe(stream);
    //   }
  	// }
    send(req, reqPath, {
      root: root,
      ...
      if (e.status === 404 && !REGEXP_FILEPATH.test(reqPath)) {
...
function useragent_supports_es6(req) {
  if (!req) return;
	let ua = req.useragent,
		...
		[majorVersion, minorVersion] = versionSplit.map(v => {
			return v
				? parseInt(v, 10)
				: -1
		});
	return (browser === 'Chrome' && majorVersion >= 49) || (browser === 'Chromium' && majorVersion >= 49) || (browser === 'Opera' && majorVersion >= 36) || (browser === 'Vivaldi' && majorVersion >= 1) || (browser === 'Safari' && majorVersion >= 10) || (browser === 'Edge' && (majorVersion > 15 || (majorVersion === 15 && minorVersion >= 15063))) || (browser === 'Firefox' && majorVersion >= 51);
}

function resolvePushManifests(src, dist) {
  return new Promise(function(resolve, reject) {
    jsonfile.readFile(path.join(src, 'push-manifest.json'), function(err, manifest) {
      let ret = { development: {} , production: {} };
      if (err) {
        console.warn(`no global push-manifest found`);
        return ret;
      }
      ret.development = manifest;
      for (const build in dist) {
        try {
          ret.production[build] = mergeOptions(manifest, jsonfile.readFileSync(path.join(dist[build], 'push-manifest.json')));
        } catch (e) {
          console.warn(`'${build}' build: no push-manifest`);
          ret.production[build] = {};
        }
      }
      resolve(ret);
    })
